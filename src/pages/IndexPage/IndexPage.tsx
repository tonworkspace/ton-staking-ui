...
import { getLevel, levels } from '@/gameData';
...
// Extract level names and min points from the levels array in gameData.ts
const levelNames = levels.map(level => level.name);
const levelMinPoints = levels.map(level => level.minBalance);
...
const DAILY_MINING_RATE = 4; // 3000 NOVA per hour = 72000 NOVA per day = 7200% daily rate
...
export const IndexPage: React.FC = () => {
  ...
  // Now use playerBalance after it's declared
  const levelInfo = getLevel(playerBalance);
  ...
  const calculatePerSecondRate = (amount: number, stakingMultiplier: number = 1) => {
    const dailyRate = amount * DAILY_MINING_RATE;
    return (dailyRate * stakingMultiplier) / 86400;
  };
  ...
  // Move getTimeEstimate inside the component
  const getTimeEstimate = (currentBalance: number, currentLevelIndex: number): string => {
    if (currentLevelIndex >= levelMinPoints.length - 1) return "Max Level";
    
    const pointsNeeded = levelMinPoints[currentLevelIndex + 1] - currentBalance;
    if (pointsNeeded <= 0) return "Ready to level up!";
    ...
    const secondsNeeded = pointsNeeded / perSecondRate;
    ...
    if (secondsNeeded < 60) {
      ...
      return `${Math.ceil(secondsNeeded / 86400)}d`;
    }
  };
  ...
  // Update the handleDeposit function to show loading state
  const handleDeposit = async (amount: number) => {
    ...
    if (amount <= 0) {
      addNotification({
        ...
        subMessage: 'Please enter a positive amount to deposit',
        icon: '⚠️'
      });
      return;
    }
    ...
    try {
      // Here you would typically check if the user has enough TON in their wallet
      // This would involve integration with a wallet or blockchain API
      const userHasSufficientBalance = true; // Replace with actual check
      
      if (!userHasSufficientBalance) {
        addNotification({
          type: 'warning',
          message: 'Insufficient Balance',
          subMessage: 'Not enough TON in your wallet',
          icon: '⚠️'
        });
        return;
      }
      ...
      // Activate mining if not already active
      if (!isMiningActive) {
        setIsMiningActive(true);
        addNotification({
          type: 'info',
          message: 'Mining Started',
          subMessage: 'Automatically activated with your deposit',
          icon: '⚡'
        });
      }
      
      // Calculate the new mining rate
      const stakingMultiplier = calculateStakingMultiplier(stakingBalance);
      const newDailyRate = calculatePerSecondRate(miningBalance + amount, stakingMultiplier) * 86400;
      
      // Show success notification with rate information
      addNotification({
        type: 'success',
        message: 'Deposit Successful',
        subMessage: `Added ${amount} TON to mining. New rate: ${formatNumber(newDailyRate)} NOVA/day`,
        icon: '💎'
      });
      
      // Optional: Track deposit in analytics or backend
  ...
  return (
    <ErrorBoundary fallback={<ErrorPage message="Something went wrong. Please try again later." />}>
      <div className="min-h-screen bg-gradient-to-b from-gray-950 via-gray-900 to-black p-custom">
        <ProfileHeader
        ...
        <List>
          {currentTab === 'home' && (
            <div className="p-2 sm:p-6 space-y-6 max-w-md mx-auto w-full mb-10">
                <div className="flex justify-between items-center p-3 bg-[#1A1A1F] rounded-lg">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-medium text-green-400">⛏️ 1% Daily</span>
            ...
            <div className="flex flex-col items-center mt-10">
                ...
                <div className="flex items-center gap-1 text-center">
                  <div className="text-6xl font-bold mb-2">
                    <SmoothNumber 
                      ...
                      duration={8000} // Adjust animation duration
                      decimals={2}  // Match the precision we're using
                    />
                  </div>
              ...
              className="flex items-center gap-2 text-[#868686] rounded-full px-4 py-2 mt-2 
                         cursor-pointer bg-white/5 hover:bg-white/10 active:bg-white/15
                         border border-white/10 hover:border-white/20
                         transition-all duration-200 transform hover:scale-[1.02]
                         shadow-lg hover:shadow-blue-500/10"
            >
              <div className="flex items-center gap-1.5">
                <span className="font-medium">{levelNames[levelIndex]}</span>
                <span className="animate-pulse">✨</span>
              </div>
              <span className="text-xs px-2.5 py-1 bg-white/10 rounded-full flex items-center gap-1">
                {levelIndex + 1}/{levelNames.length}
                <svg 
                  className="w-3 h-3 ml-0.5 transform transition-transform group-hover:translate-x-0.5" 
                  fill="none" 
                  viewBox="0 0 24 24" 
                  stroke="currentColor"
                >
            ...
            <MiningCard
              ...
              connected={!!user} // or whatever condition determines if wallet is connected
        ...
        {isDepositLoading && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-[#0A0A0C] rounded-xl p-6 flex flex-col items-center">
